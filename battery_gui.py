...
class BatteryGrouperGUI:
    def __init__(self, root):
        ...
        self.max_ir_entry.insert(0, "60")
        self.max_ir_entry.grid(row=1, column=3, padx=5, pady=5)

        # --- Data Input Frame ---
        data_frame = tk.LabelFrame(root, text=" 2. Paste Cell Data (ID, Capacity, IR) ", padx=10, pady=10)
        data_frame.pack(fill="both", expand=True, padx=10, pady=5)
        
        tk.Label(data_frame, text="Paste your data below in the format: (ID, Capacity, IR), (ID, Capacity, IR)...\nExample:\n(1, 2742, 48), (2, 2338, 58), (3, 2825, 50)").pack(anchor="w")
...
(102, 2451, 54), (103, 2383, 59), (104, 2459, 54)"""
        ...
        # --- Action Button ---
        self.run_btn = tk.Button(root, text="GENERATE PACK GROUPINGS", command=self.run_calculation, bg="#4CAF50", fg="white", font=("Arial", 12, "bold"))
    ...
    def run_calculation(self):
        ...
        # This regex looks for (number, number, number) anywhere in the text
        ...
        if not raw_data:
            messagebox.showerror("Data Error", "No valid cell data found. Please check your input.")
            return
